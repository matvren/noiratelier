...
function autoSeed() {
  ...
  if (prodCount === 0) {
    ...
    const insert = db.prepare(`INSERT INTO products (name, brand, notes, description, size, price, stock, accent, image) VALUES (?, ?, ?, ?, ?, ?, 50, ?, ?)`);
    const tx = db.transaction((rows) => rows.forEach((r, idx) => {
      const image = idx < 10 ? `/images/${idx + 1}.png` : '';
      insert.run(r.name, r.brand, r.notes, r.description, r.size, r.price, r.accent, image);
    }));
    tx(products);
